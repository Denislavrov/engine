...
//Проверка расширения файла
        ...
        foreach ($blacklist as $item) {
            if (preg_match("/$item\$/i", $_FILES['myfile']['name'])) {
                ...
                header("Location: ?page=gallery&errorMessage=php");
                exit;
            }
        }
//Проверка на тип файла
        ...
        if (move_uploaded_file($_FILES['myfile']['tmp_name'], $uploadfile)) {
            ...
            $image->save('img/gallery/' . $_FILES['myfile']['name']);

            header("Location: ?page=gallery&errorMessage=ok");
        } else {
            header("Location: ?page=gallery&errorMessage=error");
        }
    }
}